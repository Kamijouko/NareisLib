...
## 概要
NareisLib は RimWorld のポーン描画を拡張し、カスタムテクスチャをバニラおよび Human Alien Race (HAR) 資産に重ねることが可能な多層レンダリング基盤を提供します。ゲーム本体のロジックを改造する必要はありません。
...
4. **ゲーム内テスト** —— HAR と NareisLib を読み込んだ状態で RimWorld を起動し、対象ポーンを生成してレイヤー、上書き、レンダリングの順番を確認します。
...
- バニラ（ゲームのオリジナルバージョン）のノードを上書きする際は、リリース前にゲーム内の Action Manager ギズモでタグやデバッグラベルを確認してください。
