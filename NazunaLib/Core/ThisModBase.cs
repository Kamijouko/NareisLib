...
namespace NareisLib
{
    ...
    internal static class InitialModThingDefCompPatch
    {
        [HarmonyPrefix]
        public static bool Prefix(ThingDef __instance)
        {
            ...
            Log.Message("NareisLib RenderComp Resolved.");
            return true;
        }
    }
        ...
        static void Postfix(PawnRenderNodeProperties __instance)
        {
            if (__instance.workerClass != typeof(PawnRenderNodeWorker_TextureLevels))
            {
                ...
                if (!__instance.subworkerClasses.Contains(typeof(DefaultNodeSubWorker)) && !__instance.subworkerClasses.Contains(typeof(TextureLevelsToNodeSubWorker)))
                    ...
                    Log.Message("inserted! 11111111111");
            }
        }
    }*/
    ...
    internal class SetupDynamicNodesPatch
    {
        [HarmonyPostfix]
    ...
    internal class AddChildrenPatch
    {
        [HarmonyPrefix]
    ...
    internal static class StaticInitializer
    {
        ...
        {
            NareisLibBase.Instance.LateInitialize();
        }
    }



    public class NLibSettings : ModSettings
    {
    ...
    public class NareisLibBase : Mod
    {
        public NareisLibBase(ModContentPack content) : base(content)
        {
            Instance = this;
            ModStaticMethod.ThisMod = this;
            Settings = GetSettings<NLibSettings>();
            harmonyInstance = new Harmony("NareisLib.kamijouko.nazunarei");
            harmonyInstance.PatchAll();
            Log.Message("[NareisLib] PatchAll done.");
        }

        public Harmony harmonyInstance;

        public static NareisLibBase Instance;

        public static NLibSettings Settings;

        public override string SettingsCategory() => "NareisLib Settings";
        ...
        public override void DoSettingsWindowContents(Rect inRect)
        {
            ...
            listing.CheckboxLabeled(
                ...
                ref Settings.apparelLevelsDisplayToggle,
        ...
        internal void LateInitialize()
        {
            try
            {
                LongEventHandler.QueueLongEvent(LoadInitialize, "resolving all plan defs", true, null);
            }
            catch (Exception e)
            {
    ...
    public class ThisModBase : ModBase
    {
        ...
        public ThisModBase()
        {
            ...
            NareisLibBase.Instance.harmonyInstance.PatchAll();
            Log.Message("[NareisLib] PatchAll done.");
            

            var mi = AccessTools.Method(typeof(PlayDataLoader), "DoPlayLoad");
            var info = Harmony.GetPatchInfo(mi);
            if (info != null)
                foreach (var p in info.Postfixes) Log.Warning($"DoPlayLoad postfix by {p.owner} prio={p.priority}");

            var mi2 = AccessTools.Method(typeof(PawnRenderTree), "SetupDynamicNodes");
            var info2 = Harmony.GetPatchInfo(mi2);
            if (info2 != null)
                foreach (var p in info2.Postfixes) Log.Warning($"SetupDynamicNodes postfix by {p.owner} prio={p.priority}");

            var mi3 = AccessTools.Method(typeof(ThingDef), "ResolveReferences");
            var info3 = Harmony.GetPatchInfo(mi3);
            if (info3 != null)
                foreach (var p in info3.Postfixes) Log.Warning($"ResolveReferences postfix by {p.owner} prio={p.priority}");
